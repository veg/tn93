...
const char help_msg[]
    = "read a FASTA MSA and a reference sequence and output \n"
      ...
      "  -o OUTPUT                direct the file with recorded differences "
      "from reference to a file named OUTPUT (default=stdout)\n"
      "  -r REFERENCE             requried file for the reference sequence "
      "(i.e. the sequence from which the differences are computed)\n"
      "  -f FORMAT                controls the format of the output "
      "(default=" TO_STR (
          DEFAULT_FORMAT) ")\n"
                          "                           json: compressed JSON "
                          "format; 'variants' : [list of variants], "
                          "'sequences' : [list of variant codes]\n"
                          "                           csv: sequence_id, "
                          "position, alternative nucleotide;\n"
                          "  -a AMBIGS                when comparing to "
                          "reference tree ambiguities as follows "
                          "(default=" TO_STR (
                              DEFAULT_AMBIG) ")\n"
                                             "                           "
                                             "informative: compare characters "
                                             "(i.e. different character => "
                                             "mismatch), except for N -- a "
                                             "4-fold misisng base;\n"
                                             "                           "
                                             "resolve: if an ambiguity is "
                                             "resolvable to match the "
                                             "reference, do not count it;\n"
                                             "                           all: "
                                             "report all differences;\n"
                                             "  -q                       do "
                                             "not report progress updates and "
                                             "other diagnostics to stderr \n"
                                             "  -c                       DO "
                                             "NOT combine all identical "
                                             "sequences into a group (default "
                                             "is TO COMBINE); only applies to "
                                             "the JSON format\n"
                                             "  FASTA                    read "
                                             "sequences to compare from this "
                                             "file (default=stdin)\n";
...
args_t::args_t (int argc, const char *argv[])
    : output (stdout), input (stdin), reference (NULL), ambig (DEFAULT_AMBIG),
      out_format (DEFULT_FORMAT), quiet (false), compress (true)
{
      ...
      if (arg[0] == '-' && arg[1] == '-')
      ...
      else if (arg[0] == '-')
        {
          if (!strcmp (&arg[1], "h"))
            ...
            parse_reference (next_arg (i, argc, argv));
          else if (arg[1] == 'f')
...
void
args_t::parse_reference (const char *str)
{
