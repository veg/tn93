...
double
computeTransformedTN93 (const unsigned long seq1, const unsigned long seq2,
                        const long firstSequenceLength,
                        const char resolutionOption, const long min_overlap,
                        const double step_penalty, double *ds = NULL)
{

  char *s1 = stringText (sequences, seqLengths, seq1);

  double thisD
      = computeTN93 (s1, stringText (sequences, seqLengths, seq2),
                     firstSequenceLength, resolutionOption, NULL, min_overlap,
                     NULL, 0.0, 0, 1, 1, NULL, NULL, -1.0),
...
void
initializeSingleSource (const unsigned long seq_count,
...
void
reportPathToSource (const unsigned long which_index, FILE *output,
                    const bool is_json, const long firstSequenceLength,
                    const char resolutionOption, const long min_overlap,
                    const double step_penalty)
{
      ...
      while (current_index >= 0)
        {
          if (is_json)
            {
              fprintf (output, ",\"%s\"",
                       stringText (names, nameLengths, current_index));
            }
          else
            {
              double d[2];
              d[1] = computeTransformedTN93 (
                  current_index, last_index, firstSequenceLength,
                  resolutionOption, min_overlap, step_penalty, d);
              dump_sequence_fasta (current_index, output, firstSequenceLength,
                                   d);
            }
...
void
relaxDistanceEstimates (const unsigned long theSequence,
                        const long firstSequenceLength,
                        const char resolutionOption, const long min_overlap,
                        const double step_penalty)
{
...
#pragma omp parallel for default(none) shared(                                \
        my_distance_estimate, nodeParents, workingNodes, distanceEstimates,   \
            step_penalty, min_overlap, resolutionOption, firstSequenceLength, \
            theSequence, left_to_do)
#else
#pragma omp parallel for default(none) shared(                                \
        my_distance_estimate, nodeParents, workingNodes, distanceEstimates)
#endif
  ...
  for (long remaining = 0; remaining < left_to_do; remaining++)
    {
      ...
      double new_estimate = computeTransformedTN93 (
          theSequence, working_index, firstSequenceLength, resolutionOption,
          min_overlap, step_penalty);
      if (new_estimate < DBL_MAX)
        {
          if (new_estimate + my_distance_estimate
              < distanceEstimates.value (working_index))
            {
              distanceEstimates.storeValue (
                  new_estimate + my_distance_estimate, working_index);
...
main (int argc, const char *argv[])
{
  ...
  long firstSequenceLength = 0, min_overlap = 1;
  ...
  if (readFASTA (F, automatonState, names, sequences, nameLengths, seqLengths,
                 firstSequenceLength)
      == 1)
    return 1;
  ...
  if (source < 0 || source >= sequenceCount)
    ...
    {
      cerr << "Using " << stringText (names, nameLengths, source)
           << " as the source" << endl;
    }

  initializeSingleSource (sequenceCount, source);

  double percentDone = 0.0, normalizer = 100. / sequenceCount,
         step_penalty = 10.;
  ...
  while (workingNodes.length ())
    {
      unsigned long add_this_node
          = workingNodes.extractMin (distanceEstimates);
      relaxDistanceEstimates (add_this_node, firstSequenceLength,
                              resolutionOption, min_overlap, step_penalty);
      if ((sequenceCount - workingNodes.length ()) * normalizer - percentDone
              > 0.1
          || workingNodes.length () == 0)
        {
          cerr << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
                  "\b\b\b"
               << setw (8) << percentDone << "% done ";
  ...
  for (long which_arg = 8; which_arg < argc; which_arg++)
    {
      reportPathToSource (atoi (argv[which_arg]), FO, is_json,
                          firstSequenceLength, resolutionOption, min_overlap,
                          step_penalty);
