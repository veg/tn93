...
const char help_msg[]
    = "merge matching (subject to ambig resolution) reads into clusters\n"
      ...
      "(default=stdout)\n"
      "  -a AMBIGS                handle ambigous nucleotides using one of "
      "the following strategies (default=" TO_STR (
          DEFAULT_AMBIG) ")\n"
                         "                           resolve: resolve "
                         "ambiguities to minimize distance (e.g.R matches "
                         "A);\n"
                         "                           average: average "
                         "ambiguities (e.g.R-A is 0.5 A-A and 0.5 G-A);\n"
                         "                           skip: do not include "
                         "sites with ambiguous nucleotides in distance "
                         "calculations;\n"
                         "                           gapmm: a gap ('-') "
                         "matched to anything other than another gap is like "
                         "matching an N (4-fold ambig) to it;\n"
                         "  -l OVERLAP               merge reads that match "
                         "over at least this many positions (an integer >0, "
                         "default=" TO_STR (
                             DEFAULT_OVERLAP) "):\n"
                                              "  -s SIZE                  "
                                              "mimimum cluster size to report "
                                              "in output (an integer >0, "
                                              "default=" TO_STR (
                                                  DEFAULT_SIZE) "):\n"
                                                                "  -d "
                                                                "COUNTS_IN_"
...
args_t::args_t (int argc, const char *argv[])
    : output (stdout), input (stdin), ambig (DEFAULT_AMBIG),
      overlap (DEFAULT_OVERLAP), cluster_size (DEFAULT_SIZE), quiet (false),
      json (false), counts_in_name (DEFAULT_COUNTS_IN_NAME)
{
      ...
      if (arg[0] == '-' && arg[1] == '-')
      ...
      else if (arg[0] == '-')
        {
          if (!strcmp (&arg[1], "h"))
            ...
            parse_size (next_arg (i, argc, argv));
          else if (arg[1] == 'a')
...
void
args_t::parse_size (const char *str)
{
