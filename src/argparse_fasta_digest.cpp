...
const char help_msg[]
    = "Read a FASTA file, a comma separated list of nucleotide binding motifs "
      "and report, for each sequence, 1-based coordinates of any motifs "
      "(nucleotide strings, 4-16 letters) found in the sequence\n"
      "\n"
      "optional arguments:\n"
      "  -h, --help               show this help message and exit\n"
      "  -o OUTPUT                write the resulting JSON file to a file "
      "named OUTPUT (default=stdout)\n"
      "  -r OPERATION             consider reverse complements "
      "(default=" TO_STR (
          DEFAULT_RC) ")\n"
                      "                           complement: for each motif, "
                      "also consider its reverse complement  \n"
                      "                           nocomplement: only consider "
                      "the motifs themselves\n"
                      "  -m MATCH                 how to match the motif "
                      "(default=" TO_STR (
                          DEFAULT_MATCH) ")\n"
                                         "                           exact: "
                                         "match the motif exactly\n"
                                         "                           oneoff: "
                                         "allow any one motif mis-match\n"
                                         "                           "
                                         "transition: allow any one motif "
                                         "mis-match that is a transition "
                                         "(A:G, C:T)\n"
                                         "  -e MOTIF_LIST            specify "
                                         "delimited nucleotide sequences to "
                                         "use as motifs\n"
                                         "                           not case "
                                         "sensitve (aaaccc is the same as "
                                         "AAACCC)\n"
                                         "                           any "
                                         "contigous set of ACGTs define a "
                                         "motif but it must be 4-12 bases in "
                                         "length\n"
                                         "  FASTA                    read "
                                         "FASTA sequences from this file "
                                         "(default=stdin)\n";
...
args_t::args_t (int argc, const char *argv[])
    : input (stdin), output (stdout), rc (DEFAULT_RC), match (DEFAULT_MATCH),
      motif_list (NULL)
{
      ...
      if (arg[0] == '-' && arg[1] == '-')
      ...
      else if (arg[0] == '-')
        {
          if (!strcmp (&arg[1], "h"))
            ...
            parse_rc_option (next_arg (i, argc, argv));
          else if (arg[1] == 'm')
            parse_match_mode (next_arg (i, argc, argv));
          else if (arg[1] == 'e')
            parse_motif (next_arg (i, argc, argv));
          else
            ERROR ("unknown argument: %s", arg);
        }
      else if (i == argc - 1)
...
void
args_t::parse_rc_option (const char *str)
{
...
void
args_t::parse_match_mode (const char *str)
{
  if (!strcmp (str, "exact"))
    {
      match = exact;
...
void
args_t::parse_motif (const char *str)
{
