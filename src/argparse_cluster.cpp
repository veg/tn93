...
const char help_msg[]
    = "read a FASTA MSA and (greedy) cluster sequences that lie within a "
      "specific distance of each other\n"
      "\n"
      "optional arguments:\n"
      "  -h, --help               show this help message and exit\n"
      "  -o OUTPUT                direct the output file with clusters to "
      "OUTPUT either \n"
      "                           JSON or sets of FASTA files representing "
      "individual clusters\n"
      "                           (default=stdout) see also -f\n"
      "  -t THRESHOLD             sequences which lie within this distance "
      "will be clustered (>=0, default=" TO_STR (
          DEFAULT_DISTANCE) ")\n"
                            "  -a AMBIGS                handle ambigous "
                            "nucleotides using one of the following "
                            "strategies (default=" TO_STR (
                                DEFAULT_AMBIG) ")\n"
                                               "                           "
                                               "resolve: resolve ambiguities "
                                               "to minimize distance (e.g.R "
                                               "matches A);\n"
                                               "                           "
                                               "average: average ambiguities "
                                               "(e.g.R-A is 0.5 A-A and 0.5 "
                                               "G-A);\n"
                                               "                           "
                                               "skip:    do not include sites "
                                               "with ambiguous nucleotides in "
                                               "distance calculations;\n"
                                               "                           "
                                               "gapmm:   a gap ('-') matched "
                                               "to anything other than "
                                               "another gap is like matching "
                                               "an N (4-fold ambig) to it;\n"
                                               "                           a "
                                               "string (e.g. RY): any "
                                               "ambiguity in the list is "
                                               "RESOLVED; any ambiguitiy NOT "
                                               "in the list is averaged \n"
                                               "                           "
                                               "(LIST-NOT LIST will also be "
                                               "averaged);\n"
                                               "  -c CLUSTER-TYPE          "
                                               "create clusters based on the "
                                               "following rules "
                                               "(default=" TO_STR (
                                                                             ...
                                                                             DEFAULT_OUTPUT_MODE) ")\n"
                                                                                                  ...
                                                                                                  "                           (an integer >0, default=" TO_STR (
                                                                                                      DEFAULT_OVERLAP) ")\n"
                                                                                                                       ...
                                                                                                                       "                           while all others will be AVERAGED (default = " TO_STR (
                                                                                                                           DEFAULT_FRACTION) ")\n"
...
args_t::args_t (int argc, const char *argv[])
    : input (stdin), ambig (DEFAULT_AMBIG),
      cluster_type (DEFAULT_CLUSTER_TYPE), output_mode (DEFAULT_OUTPUT_MODE),
      overlap (DEFAULT_OVERLAP), quiet (false), ambigs_to_resolve (NULL),
      trunk_path (NULL), resolve_fraction (DEFAULT_FRACTION),
      first_regular (false)
{
      ...
      if (arg[0] == '-' && arg[1] == '-')
      ...
      else if (arg[0] == '-')
        {
          if (!strcmp (&arg[1], "h"))
            ...
            parse_cluster (next_arg (i, argc, argv));
          else if (arg[1] == 'm')
            parse_output_mode (next_arg (i, argc, argv));
          else if (arg[1] == 'g')
...
void
args_t::parse_cluster (const char *str)
{
  if (!strcmp (str, "all"))
    {
      cluster_type = all;
...
void
args_t::parse_output_mode (const char *str)
{
  if (!strcmp (str, "json"))
    {
      output_mode = json;
