...
const char help_msg[]
    = "compute per-position coverage from an MSA and report it as a JSON\n"
      ...
      "  -o OUTPUT                direct the output JSON to a file named "
      "OUTPUT (default=stdout)\n"
      "  -a AMBIGS                handle ambigous characters using one of the "
      "following strategies (default=" TO_STR (
          DEFAULT_AMBIG) ")\n"
                         "                           average: average "
                         "ambiguities (e.g. a nucleotide R adds 0.5 to A and "
                         "G coverage for that position);\n"
                         "                           ignore: ambiguities "
                         "contribute nothing to coverage counts;\n"
                         "  -t DATATYPE              the type of data "
                         "expected (default=" TO_STR (
                             DEFAULT_FORMAT) ")\n"
                                             "                           dna: "
                                             "DNA or RNA (IUPAC);\n"
                                             "                           "
                                             ...
                                             "  -d COUNTS_IN_NAME        if "
                                             "sequence name is of the form "
                                             "'somethingCOUNTS_IN_NAMEinteger'"
                                             " then treat the integer as a "
                                             "copy number\n"
                                             "                           when "
                                             "computing coverages (a "
                                             "character, default=" TO_STR (
                                                 COUNTS_IN_NAME) "):\n"
                                                                 "  -q        "
                                                                 "            "
                                                                 "   do not "
...
args_t::args_t (int argc, const char *argv[])
    : output (stdout), input (stdin), ambig (DEFAULT_AMBIG),
      data (DEFAULT_DATA), counts_in_name (DEFAULT_COUNTS_IN_NAME),
      quiet (false)
{
