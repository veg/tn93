...
const unsigned long masks[16] = {
  0b00000000000000000000000000000011, 0b00000000000000000000000000001100,
  0b00000000000000000000000000110000, 0b00000000000000000000000011000000,
  0b00000000000000000000001100000000, 0b00000000000000000000110000000000,
  0b00000000000000000011000000000000, 0b00000000000000001100000000000000,
  0b00000000000000110000000000000000, 0b00000000000011000000000000000000,
  0b00000000001100000000000000000000, 0b00000000110000000000000000000000,
  0b00000011000000000000000000000000, 0b00001100000000000000000000000000,
  0b00110000000000000000000000000000, 0b11000000000000000000000000000000
};

int allowed_chars[256];

unsigned
patternToCode (const StringBuffer &pattern)
{

  unsigned code = 0, L = pattern.length ();
...
main (int argc, const char *argv[])
{
  ...
  auto process_motif = [&] (StringBuffer &pattern) -> void {
    ...
    if (L >= 4 && L <= 16)
        ...
        if (args.rc == complement)
          {
            ...
            for (int i = L - 1; i >= 0; i--)
              {
                rc.appendChar (
                    complement_chars[(unsigned char)pattern.getChar (i)]);
              }
      ...
      {
        throw (std::string ("Motif '") + std::string (pattern.getString ())
               + "' is not 4-16 nucleotides in length");
      }
  };
  ...
  try
    {
      ...
      automatonState = 0;
      while (long state
             = readFASTA (args.input, automatonState, names, sequences,
