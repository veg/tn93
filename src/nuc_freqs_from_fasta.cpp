...
double
computeBinomialProb (double p, long coverage, long mutation)
{
  double sum = 0.0, term_update = p / (1. - p), cov = coverage,
         current_term = pow ((1. - p), cov), idx = 0.;
...
void
handle_a_sequence (StringBuffer &current_sequence, long *resolutions,
...
main (int argc, const char *argv[])
{
  ...
  double filter_rate = -1., p_value = 0.01;

  long firstSequenceLength = 0, *freq_counts = NULL;
  ...
  if (argc >= 5)
    {
      ...
      if (filter_rate <= 0. || filter_rate >= 1.)
        {
          cerr << "Invalid background rate supplied [must be in (0,1)]: `"
               << argv[4] << "'." << endl;
          return 1;
        }
      if (argc == 6)
        {
          ...
          if (filter_rate <= 0. || filter_rate >= 1.)
            {
              cerr << "Invalid p-value supplied [must be in (0,1)]: `"
                   << argv[5] << "'." << endl;
              return 1;
            }
        }
  ...
  while (fasta_result == 2)
    {
      fasta_result
          = readFASTA (F, automatonState, names, sequences, nameLengths,
                       seqLengths, firstSequenceLength, true);
      if (fasta_result == 1)
        {
          return 1;
        }
      if (sequences_read == 0)
        {
          ...
          for (long k = 0; k < 4 * firstSequenceLength; k++)
            freq_counts[k] = 0;
        }
  ...
  if (bin_mix)
    {
      for (long s = 0; s < firstSequenceLength; s++)
        {
          long coverage = freq_counts[s * 4] + freq_counts[s * 4 + 1]
                          + freq_counts[s * 4 + 2] + freq_counts[s * 4 + 3],
               conensus = 0, consensus_id = 0;
          ...
          for (long s = 0; s < firstSequenceLength; s++)
            {
              ...
              fprintf (FO, "%ld\t%ld\t%ld\t%ld\t%ld\t%ld\n", s + 1, coverage,
                       freq_counts[s * 4], freq_counts[s * 4 + 1],
                       freq_counts[s * 4 + 2], freq_counts[s * 4 + 3]);
            }
        }
      else
          ...
          for (long s = 0; s < firstSequenceLength; s++)
            {
              ...
              for (long k = 0; k < 4; k++)
                {
                  double supp = computeBinomialProb (filter_rate, coverage,
                                                     freq_counts[s * 4 + k]);
                  if (supp < p_value)
                    {
                      fprintf (FO, "%ld\t%ld\t%ld\t%c:%g\n", s + 1, coverage,
                               freq_counts[s * 4 + k], nucs[k], supp);
                    }
                }
            }
        }
