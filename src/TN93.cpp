...
void
dump_histogram (ostream *outStream, const char *tag, unsigned long *hist)
{
  if (tag)
    {
...
int
compare_ul (const void *v1, const void *v2)
{
...
main (int argc, const char *argv[])
{
  ...
  if (readFASTA (args.input1, automatonState, names, sequences, nameLengths,
                 ...
                 args.counts_in_name, args.include_prob)
      == 1)
    return 1;
  ...
  if (args.input2)
    {
      automatonState = 0;
      if (readFASTA (args.input2, automatonState, names, sequences,
                     nameLengths, seqLengths, firstSequenceLength, false,
                     &counts, args.counts_in_name, args.include_prob)
          == 1)
        return 1;
  ...
  double percentDone = 0., max[3] = { 0.0, 0.0, 0.0 },
         mean[3] = { 0.0, 0.0, 0.0 };
      ...
      if (args.input2 == NULL || args.do_bootstrap_two_files)
      ...
      else
        {
              ...
              if (expanded_counts[i] != last_sequence)
                {
                  ...
                  last_index = i;
                  randomized_idx.appendValue (last_sequence);
                  // cerr << randomized_idx2.value (randomized_idx2.length
                  // ()-1) << "/"
                  // << recount_2.value (recount_2.length()-1) << " (" <<
                  // counts.value (last_sequence) << ")" << endl;
                  last_sequence = expanded_counts[i];
                }
            }
          ...
          for (long sid = 0; sid < sequenceCount; sid++)
            {
              sequence_descriptors[sid]
                  = describe_sequence (stringText (sequences, seqLengths, sid),
                                       firstSequenceLength, 4UL, do_jumps);
            }
  ...
  long upperBound
      = (args.input2 && !do_fst) ? seqLengthInFile1 : sequenceCount,
...
#pragma omp parallel shared(                                                  \
        skipped_comparisons, sequence_descriptors, resolutionOption,          \
            ...
            firstSequenceLength, args, nameLengths, names, pairwise,          \
            percentDone, cerr, max, randFlag, distanceMatrix, upperBound,     \
            seqLengthInFile1, seqLengthInFile2, mean, randSeqs,               \
            weighted_counts, do_fst, randomized_fst, randomized_idx,          \
    ...
    for (long seq1 = 0; seq1 < upperBound; seq1++)
      {
        ...
        long lowerBound = cross_comparison_only ? seqLengthInFile1 : seq1 + 1L,
             ...
             instances1 = randomized_fst ? recounts.value (seq1)
                                         : counts.value (mapped_id);
        ...
        if (!cross_comparison_only)
          {
            if (instances1 > 1)
              {
                ...
                unsigned long self_v_self
                    = (instances1 * (instances1 - 1L)) >> 1;
            ...
            if (report_self)
              {
                // if this is a single file run and -0 selected, then report
                    ...
                    if (args.format == csvn)
                      {
                        ...
                        int written = snprintf (
                            link_buffer, 1024, "%ld%c%ld%c%g\n", mapped_id,
                            args.delimiter, mapped_id, args.delimiter, 0.0);
        ...
        for (unsigned long seq2 = lowerBound; seq2 < sequenceCount; seq2++)
          {
            long mapped_id2 = randomized_fst
                                  ? randomized_idx.value (seq2)
                                  : (randSeqs ? randSeqs[seq2] : seq2),
                 which_bin = 0,
                 weighted_count
                 = instances1
                   * (randomized_fst ? recounts.value (seq2)
            ...
            double thisD
                = sequence_descriptors
                      ? computeTN93 (
                            s1, stringText (sequences, seqLengths, mapped_id2),
                    ...
                    if (args.format == csv)
                      {
                        char float_buffer[128];
                        unsigned written
                            = snprintf (float_buffer, 128, "%g", thisD);

                        local_buffer.appendBuffer (n1, n1L);
                        local_buffer.appendChar (args.delimiter);
                        local_buffer.appendBuffer (
                            stringText (names, nameLengths, mapped_id2),
                            stringLength (nameLengths, mapped_id2));
                        local_buffer.appendChar (args.delimiter);
                        local_buffer.appendBuffer (float_buffer, written);
                        local_buffer.appendChar ('\n');
                      }
                        ...
                        if (args.format == csvn)
                          {
                            ...
                            int written = snprintf (
                                link_buffer, 1024, "%ld%c%ld%c%g\n", mapped_id,
                                args.delimiter, mapped_id2, args.delimiter,
                                thisD);
                            local_buffer.appendBuffer (link_buffer, written);
                          }
                        else
...
#pragma omp critical(dist_matrix)
                            {
                              distanceMatrix[mapped_id * sequenceCount
                                             + mapped_id2]
                                  = thisD;
                              distanceMatrix[mapped_id2 * sequenceCount
                                             + mapped_id]
                                  = thisD;
                            }
                          }
...
#pragma omp critical(progress)
            {
              if (current_pair_index * 100. / pairwise - percentDone > 0.1
                  || seq1 == (long)sequenceCount - 1)
                {
                  ...
                  cerr << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
                          "\b\b\b\b\b"
                       ...
                       << foundLinks << " links found, " << setw (12)
                       << std::setprecision (3)
                       << current_pair_index / difftime (after, before)
                       << " evals/sec)";
...
#pragma omp critical(hist)
  ...
  if (args.hamming_skip)
    {
      (*outStream)
          << "\t\"Note\" : \"Distance histogram and maximum distance might be "
             "inaccurate when using the -H (Hamming skip) option.\","
          << endl;
    }
  ...
  (*outStream) << "\t\"Comparisons accounting for copy numbers \" :"
               << (weighted_counts[0] + weighted_counts[1]
                   + weighted_counts[2])
               << ',' << endl;
  ...
  if (do_fst)
    {
      ...
      (*outStream) << "\t\"Mean distance [combined]"
                   << "\" : " << meta << ',' << endl;
      (*outStream) << "\t\"F_ST\" : " << pi_D / (intra_pop + pi_D) << ','
                   << endl;
