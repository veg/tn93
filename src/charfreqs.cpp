...
void
handle_a_sequence (StringBuffer &current_sequence, double *resolutions,
                   const long firstSequenceLength, long instance_count,
                   bool ignore_ambigs)
{
  long resolution_index = 0;
  const unsigned char *s
      = (const unsigned char *)current_sequence.getString ();
      ...
      if (ignore_ambigs == false)
        {
          local_instance_count
              = instance_count * resolution_count (s[c], false);
        }
...
void
handle_a_sequence_aa (StringBuffer &current_sequence, double *resolutions,
                      const long firstSequenceLength, long instance_count,
                      bool ignore_ambigs)
{
  long resolution_index = 0;
  ...
  for (long c = 0; c < firstSequenceLength; c++, resolution_index += 20)
    {
      ...
      if (ignore_ambigs == false)
        {
          local_instance_count
              = instance_count * resolution_count (s[c], true);
        }
      for (long k = 0; k < 20; k++)
        {
...
void
exportJSON (FILE *out, long rows, long columns, const double *counts,
            bool do_prot)
{
  fprintf (out, "{");
              ...
              if (counts[r * columns + c] > 0.0)
                {
                  if (first)
                    ...
                    {
                      fprintf (out, ", \"%c\" : %g", unmap_char (c, do_prot),
                               counts[r * columns + c]);
                    }
                }
            }
...
main (int argc, const char *argv[])
{
  ...
  long sequences_read = 0L, sequence_copies = 0L, firstSequenceLength = 0L,
      ...
      if (sequences_read == 0)
        {
          ...
          for (long k = 0; k < dim * firstSequenceLength; k++)
            freq_counts[k] = 0.;
        }

      if (args.data == protein)
        handle_a_sequence_aa (sequences, freq_counts, firstSequenceLength,
                              copy_count.value (0), args.ambig == skipover);
      else
  ...
  if (args.quiet == false)
    {
      cerr << endl
           << "{\"unique\" : " << sequences_read
           << " , \"total\" : " << sequence_copies << "}" << endl;
    }
  ...
  exportJSON (args.output, firstSequenceLength, dim, freq_counts,
              args.data == protein);
  if (freq_counts)
    delete[] freq_counts;
