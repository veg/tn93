...
public:
  ...
  void
  remove_a_cluster (unsigned long cluster_id)
  {
  ...
  void
  merge_clusters (unsigned long id1, unsigned long id2)
  {
    if (id1 != id2 && id1 < allocated_clusters && id2 < allocated_clusters
      ...
      {
        cerr << "Requested the merger using an invalid cluster " << id1
             << " and " << id2 << endl;
        exit (1);
      }
  }
  ...
  Vector *
  append_a_cluster (unsigned long sequence_id)
  {
        ...
        if (cluster_count == allocated_clusters)
          {
            unsigned long new_allocation
                = allocated_clusters
                  + (allocated_clusters > 512UL ? (allocated_clusters >> 2)
                                                : 128UL);
...
main (int argc, const char *argv[])
{
  ...
  if (readFASTA (args.input, automatonState, names, sequences, nameLengths,
                 seqLengths, firstSequenceLength)
      == 1)
    return 1;
              ...
              try
                {
                  double distance = computeTN93 (
                      ...
                      resolutionOption, NULL, args.overlap, NULL,
                      HISTOGRAM_SLICE, HISTOGRAM_BINS, 1L, 1L,
                      &sequence_descriptors[seq1], &sequence_descriptors[seq2],
                      -1.0);
                  ...
                  if (distance <= args.distance)
                    {
                      ...
                      for (unsigned long current_cluster_seq = 1UL;
                        ...
                        {
                          unsigned long csi
                              = current_cluster->value (current_cluster_seq);
                          double d = computeTN93 (
                              test_sequence,
                              stringText (sequences, seqLengths, csi),
                              firstSequenceLength, resolutionOption, NULL,
                              args.overlap, NULL, HISTOGRAM_SLICE,
                              HISTOGRAM_BINS, 1L, 1L,
                              &sequence_descriptors[seq2],
                              &sequence_descriptors[csi], -1.0);
                  ...
                  // cerr << "removing " << seq2 << " : " << remaining.size()
                  // << " / " << current_cluster->length() << endl;
                  ...
                  if (!args.quiet)
                    {
                      ...
                      if (dt > 0.)
                        {
                          percentDone = (sequenceCount - remaining.size ())
                                        * 100. / sequenceCount;
                          cerr << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
                                  "\b\b\b\b\b\b\b\b\b"
                               ...
                               << clusters.size () << " clusters created, "
                               << setw (12) << std::setprecision (3)
                               << ((double)sequenceCount - remaining.size ())
                                      / dt
          ...
          if (!args.quiet)
            {
              ...
              if (dt > 0.)
                {
                  percentDone = (sequenceCount - remaining.size ()) * 100.
                                / sequenceCount;
...
#pragma omp parallel shared(clusters, sequence_descriptors, resolutionOption, \
                                ...
                                firstSequenceLength, args, nameLengths,       \
                                names, percentDone, cerr)
          for (unsigned long cc = 0UL; cc < clusters.size (); cc++)
            {
              ...
              try
                {
                  for (unsigned long seq = 0UL; seq < this_cluster->length ();
                       seq++)
                    {
                      ...
                      double d = computeTN93 (
                          ...
                          stringText (sequences, seqLengths, csi),
                          firstSequenceLength, resolutionOption, NULL,
                          args.overlap, NULL, HISTOGRAM_SLICE, HISTOGRAM_BINS,
                          1L, 1L, &sequence_descriptors[seq1],
                          &sequence_descriptors[csi], -1.0);
          ...
          if (join_to.empty ())
          ...
          else
            {
              ...
              while (cluster_iterator != join_to.end ())
                {
                  // cout << "Merging " << first << " and " <<
                  // *cluster_iterator << " on " << seq1 << endl;
              ...
              if (dt > 0.)
                {
                  ...
                  cerr << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
                       ...
                       << setw (8) << percentDone << "% (" << setw (8)
                       << clusters.size () << "[" << setw (8) << mergers
                       << "] clusters created [merged], " << setw (12)
                       << std::setprecision (3)
                       << ((double)sequenceCount - remaining.size ()) / dt
                       << " sequences clustered/sec)";
      ...
      for (unsigned long cid = 0; cid < clusters.size (); cid++)
        {
          ...
          for (unsigned long p = 0; p < this_cluster->length (); p++)
            {
              ...
              unsigned char *s = (unsigned char *)stringText (
                  sequences, seqLengths, this_cluster->value (p));
          ...
          dump_fasta (stringText (sequences, seqLengths, max_id),
                      firstSequenceLength, print_to, false);
          ...
          if (args.trunk_path)
            {
              ...
              if (!print_to)
                {
                  cerr << "Failed to open '" << path_buffer << "' for writing"
                       << endl;
                  exit (0);
                }
            }
