...
main (int argc, const char *argv[])
{
  ...
  if (readFASTA (args.input, automatonState, names, sequences, nameLengths,
                 seqLengths, firstSequenceLength, false, NULL, ':', 1.0,
                 !args.quiet)
      == 1)
    return 1;
  ...
  if (!args.quiet)
    {
      cerr << "Read " << sequenceCount - 1 << " sequences of length "
           << firstSequenceLength << "nt from input file " << endl;
    }
  ...
  for (long seq_id = 1L; seq_id < sequenceCount; seq_id++)
    {
      ...
      if (!args.quiet
          && (seq_id * 100. / sequenceCount - percentDone > 0.1
              || seq_id == (long)sequenceCount - 1))
        {
          {
            time (&after);
            percentDone = seq_id * 100. / sequenceCount;
            cerr << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
                    "\b\b"
                    "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
                    "\b\b"
                    "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\bProgr"
                    "ess"
                    ":"
                 << setw (8) << percentDone << "% (" << setw (12)
                 << std::setprecision (3) << seq_id / difftime (after, before)
                 << " seqs/sec)";
      ...
      computeDifferences (ref_seq, seq_data, firstSequenceLength,
                          resolutionOption, *differences, ref_gaps,
                          &sequence_descriptors[seq_id]);

      auto intert_result
          = pattern_map.insert ({ differences, pattern_map.size () });
      ...
      if (args.compress)
        {
              ...
              for (long i = 0; i < p->length (); i++)
                {
                  ...
                  auto intert_result = differences_map.insert (
                      { mut, differences_map.size () });
                  bool did_insert = std::get<1> (intert_result);
                  long index;
                  if (did_insert)
                    {
          ...
          dump_fasta (ref_seq, firstSequenceLength, args.output, false, false,
                      0L, 0L);
          fprintf (args.output, "\",\"mutations\" : [");
          auto mutation_string = unique_differences.begin ();
          fprintf (args.output, "\"%s\"", (*mutation_string).c_str ());
          mutation_string++;
          std::for_each (mutation_string, unique_differences.end (),
                         [&] (const std::string &s) {
                           fprintf (args.output, ",\"%s\"", s.c_str ());
                         });
          fprintf (args.output, "], \"patterns\" : [");
          bool comma = false;
          std::for_each (recoded_patterns.begin (), recoded_patterns.end (),
                         [&] (const std::vector<long> &pattern) {
                           if (comma)
                             fprintf (args.output, ",");
          ...
          comma = false;
          std::for_each (copies.begin (), copies.end (),
                         [&] (const std::vector<long> &pattern) {
                           if (comma)
                             fprintf (args.output, ",");
                           long comma_inner = false;
                           ...
                           std::for_each (
                               ...
                               [&] (const long mi) {
                                 ...
                                 fprintf (args.output, "\"%s\"",
                                          stringText (names, nameLengths, mi));
                                 comma_inner = true;
                               });
                           fprintf (args.output, "]");
                           comma = true;
