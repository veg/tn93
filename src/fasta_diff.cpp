  ...
  try
    {
      ...
      /*auto compare_records = [&] (const std::string & id1, const char*
      seq_value) -> int { std::map<std::string, std::string>::iterator it =
      sequences_to_add.find (id1); if (it !=  sequences_to_add.end()) { if
      (args.checks == id) { return 1;
              }
      ...
      if (args.checks == argparse::sequence)
        {
          if (args.op == argparse::remove)
            ...
            {
              throw (std::string ("sequence matching mode MUST also use the "
                                  "remove operation"));
            }
        }

      while (long state
             = readFASTA (args.input_add, automatonState, names, sequences,
                          nameLengths, seqLengths, firstSequenceLength, true))
              ...
              if (sequence_deduplication)
                {
                  ...
                  sequences_to_add[sequences.getString ()]
                      = names.getString ();
                }
              else
                {
                  ...
                  std::string check_existing
                      = sequences_to_add[names.getString ()];
                  sequences_to_add[names.getString ()]
                      = sequences.getString ();
                  if (current_size == sequences_to_add.size ())
                    {
                      if (check_existing
                          == sequences_to_add[names.getString ()])
                        {
                          // cerr << std::string (names.getString()) + " is not
                          // a unique sequence ID in the master file" << endl;
                          non_unique_ids++;
                        }
                      else
                        {
                          throw (std::string (names.getString ())
                                 + " is not a unique sequence ID in the "
                                   "master file and different instances of "
                                   "this name have different sequences");
                        }
                    }
                }
      ...
      long sequences_to_add_count = sequences_to_add.size (), duplicates = 0L,
           master_sequences = 0L, output = 0L;

      auto echo_fasta_sequence
          = [&] (const char *id, const char *data, FILE *where) -> void {
        output++;
        fprintf (where, ">%s\n%s\n", id, data);
      };
      ...
      */
      ...
      while (long state
             = readFASTA (args.input_master, automatonState, names, sequences,
                          nameLengths, seqLengths, firstSequenceLength, true))
        {
          if (state == 2 || state == 3)
            {
              ...
              bool records_different, ids_different;
              if (!sequence_deduplication)
                {
                      ...
                      if (args.checks == argparse::id_and_sequence)
                        {
                          records_different
                              = it->second
                                != std::string (sequences.getString ());
                        }
                      else
                        {
              ...
              if (!records_different)
                { // not unique
                  ...
                  if (args.op == argparse::add)
                    {
                      echo_fasta_sequence (master_id.c_str (),
                                           sequences.getString (),
                                           args.output);
                    }
      ...
      if (args.op != argparse::remove)
        {
          for (std::pair<std::string, std::string> it : sequences_to_add)
            {
              echo_fasta_sequence (it.first.c_str (), it.second.c_str (),
                                   args.output);
            }
        }
      ...
      /*
      if (!args.quiet) {
          cerr << "Read " << sequences_to_add.size() << " sequences from the
      FASTA file" << endl;
      }*/
    }
