...
struct sequence_gap_structure
{
  ...
  long first_nongap, last_nongap, resolved_start, resolved_end, total_gaps,
      total_ambigs;
...
double computeTN93 (const char *s1, const char *s2, const unsigned long L,
                    const char matchMode, const long *randomize,
                    const long min_overlap, unsigned long * = NULL,
                    const double = 0.0, const unsigned long cnt = 0,
                    const long count1 = 1, const long count2 = 1,
                    const sequence_gap_structure * = NULL,
                    const sequence_gap_structure * = NULL,
                    const double threshold = -1.0);

long computeDifferences (const char *s1, const char *s2, const unsigned long L,
                         const char matchMode, Vector &storage,
                         const sequence_gap_structure * = NULL,
                         const sequence_gap_structure * = NULL);
...
int readFASTA (FILE *F, char &automatonState, StringBuffer &names,
               StringBuffer &sequences, Vector &nameLengths,
               Vector &seqLengths, long &firstSequenceLength,
               bool oneByOne = false, Vector *sequenceInstances = NULL,
               char sep = ':', double include_prob = 1.0,
               bool show_progress = false);
void dump_sequence_fasta (unsigned long index, FILE *output,
                          long firstSequenceLength, double *d = NULL,
                          bool = false, unsigned long from = 0L,
                          unsigned long to = 0L);
...
void dump_fasta (const char *source, const long sequence_length, FILE *output,
                 bool newln = true, bool = false, unsigned long from = 0L,
                 unsigned long to = 0L);
...
inline void
unpack_difference (long diff, long &location, unsigned &alt)
{
  location = diff >> 8;
  alt = diff & 0xff;
}

extern StringBuffer names, sequences;

extern unsigned char *resolveTheseAmbigs;

extern double resolve_fraction;

extern Vector nameLengths, seqLengths, workingNodes, nodeParents;
